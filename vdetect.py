    ...
    while True:
        ...
        try:
            if count < 30:
                ...
                #
                #     mediapath="C:\\Users\\shahana kp\\PycharmProjects\\college_violence\\media\\"
                #
                #     for i in lkframes:
                #
                ...
                #         cv2.imwrite(ms,i)
                #
                #         qry="INSERT INTO `myapp_violencesub` (`STUDENT_id`,`VIOLENCE_id`) VALUES ('"+"/media/"+ filename +"','"+str(id)+"')"
                #
                #         db.insert(qry)
                ...
                if f== True:
                    ...
                    mediapath = "C:\\Users\\Shamil\\PycharmProjects\\AI_CCTV\\media\\"
                    from datetime import datetime
                    filename = datetime.now().strftime("%Y%m%d%H%M%S") + ".jpg"

                    ms = mediapath + filename
                    db = Database()
                    id = db.insert("INSERT INTO `myapp_violence` (`date`,`photo`,`time`) VALUES (CURDATE(),'" + "/media/" + filename + "',CURTIME())")
                    ...
                    for i in lkframes:
                        ...
                        print("aaaaa", totface)
                        for u in range(0, totface):
                            print("inside check")
                            res = face_recognition.compare_faces(knownimage, others_face_encoding[u], tolerance=0.45)
                            print(res, "helllo")
                            l = 0
                            for j in res:
                                if j == True:
                                    print(knownids[l], "found")
                                    qryChk = "SELECT * FROM `myapp_violencesub` where `STUDENT_id`='"+str(knownids[l])+"' and `VIOLENCE_id`='"+str(id)+"'"
                                    ...
                                    if len(resChk)>0:
                                        ...
                                        qry = "INSERT INTO `myapp_violencesub` (`STUDENT_id`,`VIOLENCE_id`) VALUES ('" + str(knownids[l]) + "','" + str(id) + "')"
                                        db.insert(qry)
...
res = main_fight("C:\\Users\\Shamil\\Desktop\\project doc\\fightnew.mp4")
print(res)
